...
from database.models import Quest, QuestObjective, PlayerQuest, Character
...
async def check_objective_progress(character_id: int, guild_id: int, event_type: str, event_data: dict):
    """
    Called when events happen (kill, travel, talk, etc.).
    Checks if any active player quest objectives match and updates progress.
    """
    async with get_db() as db:
        ...
        for pq in player_quests:
            ...
            for obj in objectives:
                ...
                if event_type == "travel_to" and obj.objective_type == "travel_to":
                    target_loc = event_data.get("location_id")
                    if target_loc and target_loc == obj.target_location_id:
                        progress[obj_key] = target
                        changed = True

                elif event_type == "talk_to_npc" and obj.objective_type == "talk_to_npc":
                    npc_id = event_data.get("npc_id")
                    if npc_id and npc_id == obj.talk_npc_id:
                        progress[obj_key] = target
                        changed = True

                elif event_type == "kill_enemy" and obj.objective_type == "kill_enemy":
                    enemy_type = event_data.get("enemy_type", "")
                    if obj.target_enemy_type and obj.target_enemy_type.lower() in enemy_type.lower():
                        progress[obj_key] = current + 1
                        changed = True

                elif event_type == "kill_npc" and obj.objective_type == "kill_npc":
                    npc_id = event_data.get("npc_id")
                    if npc_id and npc_id == obj.target_npc_id:
                        progress[obj_key] = current + 1
                        changed = True

                elif event_type == "collect_item" and obj.objective_type == "collect_item":
                    item = event_data.get("item_name", "")
                    if obj.item_name and obj.item_name.lower() == item.lower():
                        progress[obj_key] = min(target, current + event_data.get("qty", 1))
                        changed = True

                elif event_type == "explore_area" and obj.objective_type == "explore_area":
                    loc_id = event_data.get("location_id")
                    if loc_id and loc_id == obj.explore_location_id:
                        progress[obj_key] = target
                        changed = True

                elif event_type == "reach_level" and obj.objective_type == "reach_level":
